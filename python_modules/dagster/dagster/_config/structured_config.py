...
def _convert_pydantic_descriminated_union_field(pydantic_field: ModelField) -> Field:
    ...
    if not pydantic_field.sub_fields_mapping or not all(
        issubclass(pydantic_field.type_, Config)
        for pydantic_field in pydantic_field.sub_fields_mapping.values()
    ):
    ...
    dagster_config_field_mapping = {
        descriminator_value: infer_schema_from_config_class(
            field.type_, ignore_pydantic_fields={pydantic_field.field_info.discriminator}
        )
        for descriminator_value, field in pydantic_field.sub_fields_mapping.items()
    }
...
def infer_schema_from_config_class(
    ...
    ignore_pydantic_fields: Optional[Set[str]] = None,
) -> Field:
    """
    ...
    check.param_invariant(
        _safe_is_subclass(model_cls, Config),
    ...
    for pydantic_field_name, pydantic_field in model_cls.__fields__.items():
        if ignore_pydantic_fields and pydantic_field_name in ignore_pydantic_fields:
            continue
